...
# "Reports Built" badge. The task runs this; this runs the Python script and
...
#       -Argument "-NoProfile -ExecutionPolicy Bypass -File `"$PWD\scripts\update_reports_built_badge.ps1`""
#   $trigger = New-ScheduledTaskTrigger -Daily -At 12:00
#   Register-ScheduledTask -TaskName "Dustarr badge refresh" -Action $action -Trigger $trigger
...
Add-Content -Path $log -Value "[$stamp] refreshing the Reports Built badge" -Encoding utf8

try {
    Set-Location $root
    $output = & py -3 "scripts\update_reports_built_badge.py" 2>&1
    $exit = $LASTEXITCODE
